  ...
  <v-container max-width="1100">
    ...
    >
      ...
      <template #[`item.in_office`]="{ item }">
        <v-checkbox-btn
          :model-value="item.in_office"
          @update:model-value="toggleInOffice(item, $event)"
        />
      </template>
      ...
      <v-card :title="dialogTitle">
        <v-card-text>
          <v-form v-model="formValid">
            ...
            <v-checkbox
              v-model="form.inOffice"
              :disabled="viewing"
              :error="!!fieldErrors.in_office"
              hide-details
              label="In Office"
            />

            <v-date-input
              v-model="form.outUntilDate"
              clearable
              :disabled="viewing"
              :error="!!fieldErrors.out_until"
              label="Out Until (date)"
            />
            ...
            <v-menu v-model="timeMenu" :close-on-content-click="false">
              <template #activator="{ props: menuProps }">
                <v-text-field
                  v-bind="menuProps"
                  v-model="form.outUntilTime"
                  clearable
                  :disabled="viewing"
                  label="Out Until (time)"
